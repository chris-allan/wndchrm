...
void ShowHelp()
{
	...
	printf("F[:Proj] - Feature reduction method.  A different projection method can be specified from the filter method.\n");
	printf("  If two methods are specified, the first method will be used as a filter only (i.e. features reduced without modifying their values).\n");
	printf("  The above -fN parameter is used by filter methods to select the top N fraction of features. It does not apply to projector-only methods.\n");
	printf("    -FFischer - Default. Ratio of inter-class and intra-class variances. Discrete or unbiased ordered classifications. Projector uses ratios as feature weights.\n");
	printf("    -FPearson - Pearson correlations b/w class values and feature values. Ordered (numeric) classes only. Projector uses Pearson as feature weights.\n");
	printf("    -FmRMR - Hanchuan Peng's minimum Redundancy Maximum Relevance scoring algorithm.\n");
	printf("    -FLDA - Linear Discriminant Analysis. Projector only.  Can be used on its own or together with a filter (e.g. -FFischer:LDA).\n");
