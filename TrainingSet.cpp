...
int TrainingSet::AddSample(signatures *new_sample)
{
   ...
   /* check if the sample can be added */
	...
	if ( (feature_vec_version && feature_vec_version != new_sample->version) || (feature_vec_type && feature_vec_type != new_sample->feature_vec_type) ) {
	...
	// Store the sample class and value in case its different in the file
		...
		if (res > 0) {
			...
			if ( (feature_vec_version && feature_vec_version != sample->version) || (feature_vec_type && feature_vec_type != sample->feature_vec_type) ) {
				catError ("ERROR: Adding sample #%d, from '%s' with feature vector version %d.%d to training set with feature vector versions %d.%d.\n",
					samp_index, sample->GetFileName(buffer), sample->version, sample->feature_vec_type, feature_vec_version, feature_vec_type);
				catError ("Delete .fit and .sig files generated by older versions of wndchrm and try again.\n");		
				return (INCONSISTENT_FEATURE_VECTORS);
			}
			...
			if (sample->count != signature_count && signature_count > 0) {
				catError ("ERROR: Sample #%d, from '%s' has %d features, which does not match previous samples with %d features.\n",
					samp_index, sample->GetFileName(buffer), sample->count,signature_count);
