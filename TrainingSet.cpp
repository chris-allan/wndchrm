...
int TrainingSet::AddAllSignatures(char *filename, int rotations, int tiles)
{  DIR *class_dir;
   struct dirent *ent;
		...
		if (class_dir=opendir(buffer)) {/* read the files and make sure they are sorted */
			while (ent = readdir(class_dir)) {
				...
				/* read only the .sig files which store image feature data relevant for this run */
					...
					rot_indx = indx_x = indx_y = -1;
					if (str_ptr > ent->d_name) {
						if (sscanf (str_ptr,"_%d_%d_%d.sig",&rot_indx,&indx_x,&indx_y) != 3) {
							rot_indx = 0;
							sscanf (str_ptr,"_%d_%d.sig",&indx_x,&indx_y);
						}
					}
					if (rot_indx > -1 && indx_x > -1 && indx_y > -1 && rot_indx < rotations && indx_x < tiles && indx_y < tiles) {
//						printf("Adding sig %s\n",ent->d_name);
						strcpy(files_in_class[files_in_class_count++],ent->d_name);
					} else {
//						printf("Ignoring sig %s (%d,%d,%d tiles:%d, rotations:%d)\n",ent->d_name,rot_indx,indx_x,indx_y, tiles, rotations);
					}
				}
			}
			closedir(class_dir);
			...
			while (p_line) {
				...
				*strrchr(sig_file_name,'.')='\0';
				for (int rot_idx=0;rot_idx<rotations;rot_idx++)			
					for (int tile_y=0;tile_y<tiles;tile_y++)			
						for (int tile_x=0;tile_x<tiles;tile_x++) {
							if (rot_idx > 0)
								sprintf(files_in_class[file_index],"%s_%d_%d_%d.sig",sig_file_name,rot_idx,tile_x,tile_y);
							else
...
/* LoadFromDir
   ...
   rotations -int- the number of rotations to perform (0 or 3)
...
int TrainingSet::LoadFromDir(char *filename, int rotations, int tiles, int multi_processor, int large_set, int compute_colors, int downsample, double mean, double stddev, rect *bounding_rect, int overwrite)
...
//		printf ("rotation:%d\n",rot_index);
					...
					for (tile_index_y=0;tile_index_y<tiles;tile_index_y++) {
						...
						/* check if the features for that image were processed by another process */
							if (multi_processor) {
								if (!(sig_file=ImageSignatures->FileOpen(NULL,rot_index,tile_index_x,tile_index_y,overwrite))) {
						 ...
						 /* compute the features */						 
							...
							if (!multi_processor) AddSample(ImageSignatures);
							if (tiles>1) delete tile_matrix;
							if (multi_processor) {
    ...
    if (multi_processor)
      AddAllSignatures(filename, rotations, tiles);      
