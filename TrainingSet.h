...
#define CANT_ALLOCATE_MEMORY              -10
...
/* properties */
	...
	featuregroups_t *aggregated_feature_stats;        // CEC add - only the top level TrainingSet has this allocated by wndcharm.cpp::split_and_test
	...
	int AddClass(const char *label);                                      /* add a discrete class    */
	...
	long dendrogram(FILE *output_file, char *dataset_name, char *phylib_path, int nodes_num,double *similarity_matrix, const std::vector<std::string> &labels, unsigned short sim_method,unsigned short phylip_algorithm);
